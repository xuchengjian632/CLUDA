...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size):
        ...
        self.projection = nn.Sequential(
            ...
            nn.BatchNorm2d(2),  # 在每个channel上进行标准化
            nn.LeakyReLU(0.2),  #激活层，能解决深度神经网络（层数非常多）的“梯度消失”问题，浅层神经网络（三五层那种）才用sigmoid 作为激活函数。
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        # rearrange:按照指定的格式"b n (h d) -> b h n d"进行变换
        queries = rearrange(self.queries(x), "b n (h d) -> b h n d", h=self.num_heads)#b->batch_size;n->channels
        keys = rearrange(self.keys(x), "b n (h d) -> b h n d", h=self.num_heads)
        values = rearrange(self.values(x), "b n (h d) -> b h n d", h=self.num_heads)
        # torch.einsum:按指定格式进行张量运算
        ...
        # F.softmax:按dim指示进行归一化，dim=1按行，dim=0按列
...
class ViT(nn.Sequential):
    def __init__(self, emb_size=10, depth=3, n_classes=4, **kwargs):
        super().__init__(
            ...
            ResidualAdd(
                nn.Sequential(
                    nn.LayerNorm(1000), # 1000与数据的最后一个维度相同
                    channel_attention(),
                    nn.Dropout(0.5),
                )
            ),
