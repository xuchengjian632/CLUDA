...
def cross_subject(data, label, session_id, subject_id, category_number, batch_size, iteration, lr, momentum, log_interval):
    ...
    test_idx = subject_id
    target_data, target_label = one_session_data[subject_id], one_session_label[subject_id]
    ...
    print("Length of source _data", len(source_data))
    ...
    # index = [i for i in range(len(source_data_comb))]
    # random.shuffle(index)
    # source_data_comb = source_data_comb[index]
    # source_label_comb = source_label_comb[index]
    #
    # index = [j for j in range(len(target_data))]
    # random.shuffle(index)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='seed3', help='the dataset used for DAN, "seed3" or "seed4"')
    parser.add_argument('--norm_type', type=str, default='ele', help='the normalization type used for data, "ele", "sample", "global" or "none"')
    parser.add_argument('--batch_size', type=int, default=64, help='size for one batch, integer')
    ...
    data = np.load('SEED_raw_data.npy')
    label = np.load('SEED_raw_label.npy')
    ...
    #     for subject_id_main in range(1, 15):
    ...
    for session_id_main in range(3):
            csub.append(cross_subject(data_tmp, label_tmp, session_id_main, 0, category_number,
                                      batch_size, iteration, lr, momentum, log_interval))
    ...
    # result = np.zeros(shape=(13, 2))
    # sub = np.array(csub).reshape(3, 13)
    # sub = sub.transpose()
    # for j in range(13):
    #     result[j][0] = np.mean(sub[j])
    #     result[j][1] = np.std(sub[j])
    # print(result)
